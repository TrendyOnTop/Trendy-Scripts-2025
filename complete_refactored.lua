-- ============================================
-- COMPLETE REFACTORED SCRIPT WITH SETTINGS TABLE
-- All features preserved, organized settings, lock button only
-- ============================================
...
local function cooked(Sex3)
    if Sex3 then  
        ...
        getgenv().esp = {
            TargetOnly = getgenv().Settings.ESP.TargetOnly,
            AutoStep = true,
            CharacterSize = Vector3.new(4, 5.75, 1.5),
            CharacterOffset = CFrame.new(0, -0.25, 0),
            UseBoundingBox = false,
            PriorityColor = Color3.new(1,0.25,0.25),
            BoxEnabled = getgenv().Settings.ESP.BoxEnabled,
            BoxCorners = getgenv().Settings.ESP.BoxCorners,
            BoxDynamic = getgenv().Settings.ESP.BoxDynamic,
            BoxStaticXFactor = 1.3,
            BoxStaticYFactor = 1.6,
            BoxColor = Color3.fromRGB(255, 255, 255),
            SkeletonEnabled = getgenv().Settings.ESP.SkeletonEnabled,
            SkeletonColor = Color3.fromRGB(255, 255, 255),
            SkeletonMaxDistance = 300,
            ChamsEnabled = getgenv().Settings.ESP.ChamsEnabled,
            ...
            TextEnabled = getgenv().Settings.ESP.TextEnabled,
        ...
        getgenv().crosshair = {
            color = getgenv().Settings.Crosshair.Color,
            mode = getgenv().Settings.Crosshair.Position,
            sticky = getgenv().Settings.Crosshair.StickToTarget,
            enabled = getgenv().Settings.Crosshair.Enabled,
            spin = getgenv().Settings.Crosshair.Spin,
        }
        ...
        -- [Insert all the original functions, connections, hit effects, etc. here]
        -- The original script code continues from here...
        -- ============================================
